// CryptoSignal Pro - Engine Logic (Updated with Advanced Strategy System)

const MASTER_KEY = 'apex_master_data';
const STRAT_KEY = 'apex_strategies';
let tvWidget = null;

// Helpers to sync storage
function getStrategies() {
    return JSON.parse(localStorage.getItem(STRAT_KEY) || '[]');
}

function saveStrategies(strategies) {
    localStorage.setItem(STRAT_KEY, JSON.stringify(strategies));
    ...
    strategies.forEach((strat, index) => {
        ...
        html += `
            <div class="col-md-6 col-lg-4">
                <div class="card p-3 h-100 border border-secondary position-relative">
                    <div class="d-flex justify-content-between align-items-start mb-2">
                        <div>
                            <h6 class="fw-bold text-accent m-0">${escapeHtml(strat.name)}</h6>
                            <span class="badge bg-dark border border-info text-info me-1 mt-1">${typeBadge}</span>
                            <span class="badge ${isTop10 ? 'bg-warning text-dark' : 'bg-secondary'} me-1 mt-1">${isTop10 ? '🎯 TOP 10 COINS' : escapeHtml(strat.coin)}</span>
                        </div>
                        <span class="badge ${strat.active ? 'bg-success' : 'bg-danger'}">${strat.active ? 'RUNNING' : 'PAUSED'}</span>
                    </div>

                    ${strat.aiPrompt ? `<div class="p-2 my-2 bg-dark rounded border border-secondary small text-muted text-truncate" title="${escapeHtml(strat.aiPrompt)}"><strong>Prompt:</strong> ${escapeHtml(strat.aiPrompt)}</div>` : ''}

                    <div class="row g-1 my-2 bg-dark p-2 rounded border border-secondary small">
...
function initDefaultStrategies() {
    ...
    if (strats.length === 0) {
        const defaultData = [
            {
                name: 'BTC Top 10 Scalper',
                type: 'ai_prompt',
                aiPrompt: 'Scan top 10 market cap coins for strong breakout candles',
                coin: 'TOP10_SCAN',
                capitalPct: 15,
                leverage: 10,
                rrRatio: '1:2',
        ...
        html += `
            <div class="p-2 mb-2 rounded bg-dark border border-secondary">
                <div class="d-flex justify-content-between align-items-center mb-1">
                    <span class="fw-bold">${escapeHtml(sig.coin)}</span>
